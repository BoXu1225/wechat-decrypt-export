...
./wechat send --to 张三 --quote 0:1234 --text "好的"    # reply quoting message 0:1234 (引用)
```
...
- **Sharing the Mac with the helper:** a send uses the real keyboard focus for ~10 s. Before starting it waits until you haven't typed, clicked or scrolled for 2 s (moving the mouse is fine) (up to 30 s, else it gives up with `user_busy` — nothing happened). While it works a banner at the top of the screen says *Sending to … — please don't use the keyboard or mouse*, then *Sent to …* or *Stopped*. If you type, click, scroll or switch apps meanwhile, it stops before pressing Enter (`user_activity`) and nothing is sent; if the text was already typed it stays as an unsent draft in that chat, and the result says so. Reading chats never touches the screen. Settings: `send_idle_s`, `send_idle_timeout_s`.
- **Quote replies:** `--quote ID` (MCP: `quote`) replies to a text message as a WeChat quote. The id comes from `get_messages` / `search_messages`. The message must still be on screen (among the latest messages in the chat). The helper finds that exact bubble on the right side (theirs left, yours right), right-clicks it and clicks only the menu item that reads exactly *Quote* / *引用* — never Recall or Delete — then checks the quote preview shows that sender and text before typing. The database check confirms the reply and the quoted message.
