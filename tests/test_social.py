    ...
    def test_mcp_in_process_and_log(self):
        ...
        async def go():
            async with Client(srv) as client:
                ...
                self.assertEqual(json.loads(r.content[0].text)["count"], 1)
                r = await client.call_tool("search_favorites", {"query": "milk"})
                self.assertEqual(json.loads(r.content[0].text)["favorites"][0]["id"], 1)
                r = await client.call_tool("get_favorite", {"id": 4})
                self.assertEqual(len(json.loads(r.content[0].text)["items"]), 3)
