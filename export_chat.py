...
# 记录中不写入输出文件的内部字段
_INTERNAL_KEYS = ("packed_info_data", "media_duration", "emoji_xml")
...
def main(argv=None):
    ...
    if args.download_emoji and not (args.images or args.media):
        parser.error("--download-emoji 需要与 --images 或 --media 一起使用")
    ...
    images = (ImageExporter(cfg, download_emoji=args.download_emoji, decrypted_dir=decrypted_dir)
              if args.images or args.media else None)
