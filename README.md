...
./wechat send --to 张三 --quote 0:1234 --text "好的"    # 引用消息 0:1234 回复
```
...
- **引用回复：** `--quote ID`（MCP 参数 `quote`）以微信「引用」的形式回复一条文字消息，ID 来自 `get_messages` / `search_messages`。该消息必须仍显示在屏幕上（聊天里最近的消息）。助手会在正确的一侧找到这条气泡（对方在左、自己在右），右键后只点击文字恰好为「引用」/ *Quote* 的菜单项——绝不会点撤回或删除——并在输入前核对引用预览显示的发送者和内容。发送后在数据库确认回复和被引用的消息。
